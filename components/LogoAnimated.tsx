...
export default function LogoAnimated({ height = 38 }: { height?: number }) {
  ...
  useEffect(() => {
    // Feltételek ellenőrzése egyszer, mount-kor
    const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const weakCPU = (navigator.hardwareConcurrency ?? 4) < 4;
    const isMobile = window.innerWidth < 768;

    if (!reducedMotion && !weakCPU && !isMobile) {
      setCanAnimate(true);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Véletlenszerű időközönként triggereli az animációt (8-16 mp között)
    const scheduleNext = () => {
      const delay = 8000 + Math.random() * 8000; // 8-16 másodperc
      timerRef.current = setTimeout(() => {
        setAnimating(true);
        // Animáció után resetel (1.8s az animáció hossza)
        setTimeout(() => {
          setAnimating(false);
          scheduleNext();
        }, 1800);
